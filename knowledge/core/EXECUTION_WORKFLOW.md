...
**Actions**:
- Apply verification patterns from [VERIFICATION_PATTERNS.md](VERIFICATION_PATTERNS.md)
  - Assumption validation (explicit assumptions, edge cases)
  - Output accuracy (paths exist, commands run, syntax correct)
  - Cross-checking (documentation matches, no contradictions)
  - Edge cases (wrong environment, missing deps, scope drift)
  - Sanity checks (proportional effort, solving right problem)
- Select verification level based on task risk (Light/Standard/Deep)
