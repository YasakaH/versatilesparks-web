<arg_value>// One-shot generator for the static legal/info pages.
    // Writes files directly with fs.writeFileSync (bypassing any editor formatter)
    // so the .tsx source is guaranteed to start with `import`, not a stray backtick.
    const fs = require("fs");
...
const header = (title, desc, slug) => `import LegalLayout, {legalMetadata} from "../../components/LegalLayout";
    ...
    // Callback / Access (post-purchase landing)
    writeFile(
    "callback/page.tsx",
    header(
    "Access Your Library",
